...
from larp_egov.apps.accounts.exceptions import (
    InvalidPasswordError,
    InvalidResetPasswordSignatureError,
    WrongPasswordError,
)
from larp_egov.apps.accounts.models import UserAccount
from larp_egov.apps.accounts.services.password import PasswordService


PASSWORD_SERVICE_PATH = "larp_egov.apps.accounts.services.password.PasswordService"  # nosec
...
def test_reset_password_signature_failure(mocker, settings, exception):
    ...
    settings.LARP_EGOV_RESET_PASSWORD_EXPIRATION_DELTA = 42
    mocked_change_password = mocker.patch(f"{PASSWORD_SERVICE_PATH}.change_password")
