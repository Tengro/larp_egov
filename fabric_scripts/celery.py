...
def celery_all(ctx):
    with ctx.cd(project_path("api")):
        ctx.run("celery -A larp_egov worker -l DEBUG --beat", pty=True, replace_env=False)


@task()
def celery_worker(ctx):
    with ctx.cd(project_path("api")):
        ctx.run("celery -A larp_egov worker -l DEBUG", pty=True, replace_env=False)


@task()
def celery_beat(ctx):
    with ctx.cd(project_path("api")):
        ctx.run("celery -A larp_egov beat -l DEBUG", pty=True, replace_env=False)


@task()
def celery_clear_queue(ctx):
    with ctx.cd(project_path("api")):
        ctx.run("celery -A larp_egov purge -f", pty=True, replace_env=False)
