...
def compose_up(ctx, daemon=False):
    with ctx.cd(project_path("docker")):
        command = "docker-compose --file docker-compose-local.yml --project-name=larp_egov up"
        if daemon:
            command = f"{command} -d"
...
def compose_down(ctx, volumes=False):
    with ctx.cd(project_path("docker")):
        command = "docker-compose --file docker-compose-local.yml --project-name=larp_egov down"
        if volumes:
            command = f"{command} -v"
        ctx.run(command, pty=True, replace_env=False)
